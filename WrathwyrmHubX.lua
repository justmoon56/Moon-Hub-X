...
local Window = Fluent:CreateWindow({
    Title = "WrathWyrm-Hub-X | Evade Legacy",
    SubTitle = "v2.5.6 Made By justmoon56",
    TabWidth = 160,
...
local Tabs = {
    ...
    Random = Window:AddTab({ Title = "| Old Folder", Icon = "box" }),
...
local AssetsIcon = "rbxassetid://125943203661301"

-- Fake Stroke Layer
local backImage = Instance.new("ImageLabel")
backImage.Name = "StrokeLayer"
backImage.Parent = mainopen
backImage.Size = UDim2.new(0.65, 0, 1.08, 0)
backImage.Position = UDim2.new(0.5, 0, 0.5, 0)
backImage.AnchorPoint = Vector2.new(0.5, 0.5)
backImage.BackgroundTransparency = 1
backImage.Image = AssetsIcon
backImage.ImageColor3 = Color3.fromRGB(255, 127, 0)
...
frontImage.Size = UDim2.new(0.6,0,1,0)
...
function DFunctions.CreateButton(ButtonName,Name,Size1,Size2,ScriptLogic,CircleMode,Keybind)
    ...
    task.spawn(function()
        while task.wait(0.03) do
            gradient.Rotation=(gradient.Rotation+1)%360
            gradient.Color=(getgenv().ButtonGradients and getgenv().ButtonGradients.Background) or ColorSequence.new(Color3.new(1,1,1))
        end
    end)
    ...
    toggle.MouseButton1Click:Connect(function()
        applyShape(
            not frame:GetAttribute("IsCircle")
        )
    end)
...
function DFunctions.DestroyInvisibleBarrier()
...
function DFunctions.StartLag(ms)
    ...
    if mode == "Normal" or LagTime < 0.2 then
...
Tabs.Main:AddSection("Cactus Esp")

DConfiguration.Highlight.Cactus=false

local CactusESP={}

local Toggle=Tabs.Main:AddToggle("CactusESP", { Title="Cactus ESP", Default=false })

Toggle:OnChanged(function(value)
    DConfiguration.Highlight.Cactus=value

    if value then
        table.clear(CactusESP)
        ...
        if not found then
            Fluent:Notify({
                Title="Error",
                Content="No Cactus Here",
                Duration=2
            })
        end

    else
        for _,v in pairs(CactusESP) do
            if v then v:Destroy()
            end
        end
...
local Toggle=Tabs.Main:AddToggle("UseCola",{
    Title="Use Cola",
    Default=false,
    ...
    Callback=function(Value)
        ...
        if Value then
            ...
            DFunctions.CreateButton(
                ...
                0.15+DConfiguration.Settings.GuiScale.UseCola,
                0.1+DConfiguration.Settings.GuiScale.UseCola,
...
Tabs.Main:AddInput("UseColaSize", {
    Title = "Use Cola Size",
    Default = tostring(DConfiguration.Settings.GuiScale.UseCola),
    Placeholder = "0",
    Numeric = true,
    Finished = false,
    ...
    Callback = function(Value)
        ...
        if num then
            DConfiguration.Settings.GuiScale.UseCola = num * 0.01
        else
            DConfiguration.Settings.GuiScale.UseCola = 0
        end
...
Tabs.Main:AddButton({
        Title = "Destroy Invisible Wall",
        Description = "",
        Callback = function()

        DFunctions:DestroyInvisibleBarrier()
    end
})
...
local Toggle=Tabs.Main:AddToggle("BarrierCollision", { Title="One Pixel Barrier", Default=false })

local BarrierConnection
...
local function setBarrierCollision(state)
    ...
    for _,v in pairs(Effects:GetDescendants()) do
        if v:IsA("Model")
        and v.Name=="Barrier" then
            local box=v:FindFirstChild("BoundingBox",true)
            if box and box:IsA("BasePart") then
                box.CanCollide=state
            end
        end
    end
end
...
Toggle:OnChanged(function(state)
    setBarrierCollision(state)
    if BarrierConnection then BarrierConnection:Disconnect()
    end

    if state then
        BarrierConnection=workspace.Game.Effects.DescendantAdded:Connect(function(v)
...
Toggle:OnChanged(function(value)
    ...
    while DConfiguration.Main.Noclip and task.wait() do
...
Tabs.Misc:AddToggle("EnableEfling", {
    Title = "Efling Modified",
    Default = false,
...
local function isValidEdge(part, root)

    local Config =DConfiguration.Misc.MovementModification.Efling
    ...
    if size.Magnitude <
        Config.MinModelSize then
        return false
    end

    if size.X <= Config.MinEdgeWidth or size.Z <= Config.MinEdgeWidth then
        return false
    end
    ...
    local hitX = math.abs(localPos.X) >= (sx - 0.15)

    local hitZ = math.abs(localPos.Z) >= (sz - 0.15)

    if not (hitX or hitZ) then
        return false
    end

    local centerDist =Vector2.new(localPos.X,localPos.Z).Magnitude

    if centerDist < 2 then
        return false
    end

    local edgeHeight =root.Position.Y - part.Position.Y

    if edgeHeight < Config.MinEdgeHeight then
        return false
    end
...
local function fling(root)
    ...
    root.AssemblyLinearVelocity = Vector3.new(vel.X, Config.Power, vel.Z)
...
local Toggle=Tabs.Misc:AddToggle("BHOPToggle",{
    Title="BHOP (Button)",
    Default=false
})
    ...
    if State then
        ...
        DFunctions.CreateButton(
            ...
            function(btn)
                ...
                DConfiguration.Misc.MovementModification.BHOP.FloatingButton=
                not DConfiguration.Misc.MovementModification.BHOP.FloatingButton
                ...
                while DConfiguration.Misc.MovementModification.BHOP.FloatingButton and wait(0.1) do
                    DConfiguration.Misc.MovementModification.BHOP.Enabled=
                    DConfiguration.Misc.MovementModification.BHOP.FloatingButton
                end
...
local ChangeEmote1 = ""
local ChangeEmote2 = "" 
local ChangeCosmetics1 = "" 
local ChangeCosmetics2 = ""
...
Tabs.Info:AddSection("Info/Contributor Image")
...
local function AddCustomImage(url)

    local parent=Tabs.Info.Scroll or Tabs.Info.Container or Tabs.Info.Elements
    ...
    local fileName=HttpService:GenerateGUID(false):gsub("%-","")

    fileName=tostring(#url).."_"..
        tostring(string.byte(url,1) or 0)..".png"

    local path="ImageCache/"..fileName
    ...
    holder.Size=UDim2.new(1,-10,0,250)
    ...
    image.Image=getcustomasset(path)
...
AddCustomImage("https://od.lk/d/ODhfMjAxODk5MzNf/IMG_20260522_134241.jpg")
AddCustomImage("https://od.lk/d/ODhfMjAxODk5MzRf/noFilter.png")
AddCustomImage("https://od.lk/d/ODhfMjAxODc3NTFf/c0589f2e2545b8be2d3100540918c83fee96d3fe18cc115f3132b576c9145a3c.webp")
...
Tabs.Info:AddButton({
    ...
    Callback = function()
        setclipboard(
            "https://discord.gg/fxF4MpJHu"
        )
    end
})

Tabs.Info:AddParagraph({
    Title = "WrathWyrmHubX | Evade Legacy",
    Content = "Made By justmoon56"
})

Tabs.Info:AddParagraph({
    Title = "FluentModded",
    Content = "Made By StyearX"
})

Tabs.Info:AddParagraph({
    Title = "Alpha Tester",
    Content = "Testing By snowy_wq"
})

-- Extension

Tabs.Extension:AddAudio({
...
Tabs.Extension:AddButton({
    ...
    Callback = function()
        ...
        if success and objects and objects[1] then
            ...
            local cf = (root.CFrame * CFrame.new(0, 2, -8)) * CFrame.Angles(0, math.rad(90), 0)
            ...
            if asset:IsA("Model") then
                ...
                boxPart.Name = "InvisParts"
                boxPart.Massless = true
                boxPart.Transparency = 1
                boxPart.Anchored = true
                boxPart.CanCollide = true

                boxPart.Size = Vector3.new(10.4, 6.7, 1.2)
                boxPart.CFrame = (cf * CFrame.new(0, 1.1, 0)) * CFrame.Angles(0, math.rad(-90), 0)
                boxPart.Parent = asset

                local boundingBox = Instance.new("SelectionBox")
                boundingBox.Name = "BoundingBox"
                boundingBox.Adornee = boxPart
                boundingBox.Color3 = Color3.fromRGB(128, 255, 255)
                boundingBox.LineThickness = 0
                boundingBox.Transparency = 1
                boundingBox.Parent = asset
            end

            table.insert(SpawnedFences, asset)

            Fluent:Notify({
                Title = "Success",
                Content = "Fence Success Fully Loaded!",
                Duration = 2
            })
        else
            Fluent:Notify({
                Title = "Error",
                Content = "Failed to load model assets.",
                Duration = 2
            })
        end
    end
...
Tabs.Extension:AddButton({
    ...
    Callback = function()
        ...
        for _, v in pairs(SpawnedFences) do
        ...
        table.clear(SpawnedFences)

        Fluent:Notify({
            Title = "Success",
            Content = "Deleted " .. deleted .. " Fence",
            Duration = 2
        })
    end
})
...
Tabs.Extension:AddButton({
    ...
    Callback = function()
        ...
        if not success then
            warn("Failed to load Bloxstrap: " .. tostring(err))
            Fluent:Notify({
                Title = "Success",
                Content = "Succses Fully Loaded Bloxstrap! You Can Use It.",
                Duration = 3
            })
        end
        return true
...
Tabs.Extension:AddButton({ Title="Accurate Low Quality", Description="(Fast Flag)",
...
Tabs.Random:AddParagraph({
    Title = "Old Folder",
    Content = ""
})

Tabs.Random:AddButton({
    Title = "Lag Switch (Invisible Mode)",
    Description = "",
    Callback = function()
        loadstring(game:HttpGet("https://raw.githubusercontent.com/justmoon56/LagSwitch-NETVOID/main/MoonHub"))()
    end
})
...
local Dropdown = Tabs.Random:AddDropdown("SelectEmote", {
    Title = "Select Emote",
    Values = {"1","2","3","4","5","6"},
    Multi = false,
    Default = "1",
    Callback = function(v)
        _G.EmoteNumber = tonumber(v)
    end
})
...
Tabs.Random:AddToggle("MacroToggle", { Title = "Enable Macro", Default = false,
    ...
    Callback = function(state)
        if state then
            if not Loaded then
                ...
                loadstring(game:HttpGet("https://raw.githubusercontent.com/justmoon56/MacroGui/refs/heads/main/Macro.lua"))()

                task.wait(.2)

                if _G.CreateMacroUI then
                    _G.CreateMacroUI()
                end
            end
        end
        ...
        local emoteGui = PlayerGui:FindFirstChild("EmoteGui")
        local crouchGui = PlayerGui:FindFirstChild("CrouchGui")

        if emoteGui then
            emoteGui.Enabled = state
        end

        if crouchGui then
            crouchGui.Enabled = state
...
function updateAllTransparency()
    if emoteGui then
        local emoteBtn=emoteGui:FindFirstChildWhichIsA("ImageButton", true)

        if emoteBtn then
            emoteBtn.ImageTransparency=1
            local emoteIcon=emoteBtn:FindFirstChildWhichIsA("TextLabel", true)
            if emoteIcon then
    ...
    if crouchGui then
        local crouchBtn=crouchGui:FindFirstChildWhichIsA("ImageButton", true)
        if crouchBtn then
            crouchBtn.ImageTransparency=1
            local crouchIcon=crouchBtn:FindFirstChildWhichIsA("ImageLabel", true)
            if crouchIcon then
...
local function updateButtonSize()
    local function apply(gui)
        ...
        local btn=gui:FindFirstChildWhichIsA("ImageButton", true)
...
Tabs.Random:AddInput("TextTransparency", { Title="Emote Transparency", Default="0.6",
    ...
    Callback=function(v)
        ...
        if n then
            textTransparency=math.clamp(n,0,1)
...
Tabs.Random:AddInput("IconTransparency", { Title="Crouch Transparency", Default="0.6",
    ...
    Callback=function(v)
        ...
        if n then
            iconTransparency=math.clamp(n,0,1)
...
Tabs.Random:AddInput("MacroButtonSize", { Title="Button Size", Default="80",
...
task.spawn(function()
    ...
    repeat
        ...
        if pg then
            emoteGui=pg:FindFirstChild("EmoteGui")
            crouchGui=pg:FindFirstChild("CrouchGui")
        end

    until emoteGui and crouchGui
...
Tabs.Random:AddInput("RampX", {
    ...
    Callback = function(v)
        local n = tonumber(v)
        if n then
            rampX = math.clamp(n,1,500)
        end
    end
})

Tabs.Random:AddInput("RampY", {
    Title = "Size Y",
    Default = "20",
    Numeric = true,
    Callback = function(v)
        local n = tonumber(v)
        if n then
            rampY = math.clamp(n,1,500)
        end
    end
})

Tabs.Random:AddInput("RampZ", {
    Title = "Size Z",
    Default = "60",
    Numeric = true,
    Callback = function(v)
...
Tabs.Random:AddButton({
    ...
    Callback = function()
        ...
        ramp.Color = Color3.fromRGB(255,140,0)

        ramp.Anchored = true
        ramp.CanCollide = true

        ramp.Position = root.Position + root.CFrame.LookVector * 25
...
Tabs.Random:AddInput("ScaleNormal", {
    Title = "Uniform Scale",
    Default = "1",
    Numeric = true,
    Callback = function(v)
        local n = tonumber(v)
        if n then scaleValue = math.clamp(n,0.1,20) end
    end
})

Tabs.Random:AddInput("ScaleX", {
    Title = "Scale X",
    Default = "1",
    Numeric = true,
    Callback = function(v)
        local n = tonumber(v)
        if n then scaleX = math.clamp(n,0.1,20) end
    end
})

Tabs.Random:AddInput("ScaleY", {
    Title = "Scale Y",
    Default = "1",
    Numeric = true,
    Callback = function(v)
        local n = tonumber(v)
        if n then scaleY = math.clamp(n,0.1,20) end
    end
})

Tabs.Random:AddInput("ScaleZ", {
    Title = "Scale Z",
    Default = "1",
    Numeric = true,
    Callback = function(v)
        local n = tonumber(v)
        if n then scaleZ = math.clamp(n,0.1,20) end
    end
})

-- Universal Loader
local function loadAssetUniversal(id)
...
local function applyUniformScale()
    ...
    if spawnedModel:IsA("Model") then
        pcall(function()
            spawnedModel:ScaleTo(scaleValue)
        end)
    elseif spawnedModel:IsA("BasePart") then
        spawnedModel.Size = spawnedModel.Size * scaleValue
    end
end

local function applyAxisScale()
    if not spawnedModel then return end
...
-- auto Remove invis wall in Model + auto anchored
local function smartCollision(model)
    for _,v in pairs(model:GetDescendants()) do
        if v:IsA("BasePart") then
            
            local name = v.Name:lower()

            local isInvisible = v.Transparency >= 0.8
            local isHitbox = name:find("hitbox") 
                          or name:find("wall") 
                          or name:find("collision")

            if isInvisible or isHitbox then
                v.CanCollide = true
                v.CanTouch = true
                v.CanQuery = true
            else
...
local function spawnModel()
    local success, err = pcall(function()
        ...
        local obj = loadAssetUniversal(id)
        assert(obj, "Failed to load asset")
...
Tabs.Random:AddButton({
    ...
    Callback = applyUniformScale
})

Tabs.Random:AddButton({
    Title = "Apply Axis Scale",
    Callback = applyAxisScale
})
...
Tabs.Settings:AddButton({
        ...
        Description = "",
        Callback = function()
            loadstring(game:HttpGet("https://rawscripts.net/raw/Universal-Script-Free-private-server-works-with-any-game-193304"))()
        end
    })
...
InterfaceManager:SetFolder("WrathwyrmHubX_Legacy")
SaveManager:SetFolder("Wrathwyrm-Hub-X/Config")
FloatingButtonManager:SetFolder("WrathwyrmHubX/Floating")
